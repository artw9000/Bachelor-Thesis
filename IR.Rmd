---
title: "Bachelor thesis, IR outcome variable"
...
iris <- read_excel("C:/Users/Artur/OneDrive/Pulpit/df3.xlsx")
...
```{r}
iris_rec <- recipe(IR ~ GA + FEE + MCAP + PE + TA + MNGR + Bull_Index + SECTOR_B + SECTOR_C + SECTOR_D + SECTOR_E + SECTOR_F + SECTOR_G + SECTOR_H + SECTOR_I, data = iris)
set.seed(456)
...
final_rf %>%
  ...
  fit(IR ~ GA + FEE + MCAP + PE + TA + MNGR + Bull_Index + SECTOR_B + SECTOR_C + SECTOR_D + SECTOR_E + SECTOR_F + SECTOR_G + SECTOR_H + SECTOR_I,
...
```{r}
rf_defaults <- rand_forest(mode = "regression", trees=1000, mtry=15, min_n=60)
rf_defaults
...
```
```{r}
rf_mod <- rand_forest(trees=1000, mtry=15, min_n=60) %>% 
  set_engine("ranger") %>% 
  set_mode("regression")
```
```{r}
...
norm_recipe <- 
  recipe(
    IR ~ GA + FEE + MCAP + PE + TA + MNGR + Bull_Index + SECTOR_B + SECTOR_C + SECTOR_D + SECTOR_E + SECTOR_F + SECTOR_G + SECTOR_H + SECTOR_I, 
    data = iris
  ) %>%
...
lm <- 
  ...
  set_engine("lm") %>%
    set_mode("regression") %>%
  fit(IR ~ GA + FEE + MCAP + PE + TA + MNGR + Bull_Index + SECTOR_B + SECTOR_C + SECTOR_D + SECTOR_E + SECTOR_F + SECTOR_G + SECTOR_H + SECTOR_I, data = iris)
summary(lm$fit)
...
```{r}
test_results %>% 
  gather(model, prediction, -IR, -GA, -Bull_Index, -FEE, -MCAP, -MNGR, -PE, -SECTOR, -TA, -SECTOR_B, -SECTOR_C, -SECTOR_D, -SECTOR_E, -SECTOR_F, -SECTOR_G, -SECTOR_H, -SECTOR_I) %>% 
  ggplot(aes(x = prediction, y = IR)) + 
  geom_abline(col = "green", lty = 2) + 
  geom_point(alpha = .4) + 
  facet_wrap(~model) + theme_bw() + coord_fixed(xlim = c(0, 40))
```
...
```{r}
cars <- read_excel("C:/Users/Artur/OneDrive/Pulpit/df5.xlsx")
...
```{r}
cars_rec <- recipe(IR ~ FEE + MCAP + PE + TA + MNGR + Bull_Index + SECTOR_B + SECTOR_C + SECTOR_D + SECTOR_E + SECTOR_F + SECTOR_G + SECTOR_H + SECTOR_I, data = cars)
set.seed(456)
...
final_rf_cars %>%
  set_engine("ranger", importance = "permutation") %>%
  fit(IR ~ FEE + MCAP + PE + TA + MNGR + Bull_Index + SECTOR_B + SECTOR_C + SECTOR_D + SECTOR_E + SECTOR_F + SECTOR_G + SECTOR_H + SECTOR_I,
...
```{r}
rf_defaults_cars <- rand_forest(mode = "regression", trees=1000, mtry=14, min_n=60)
rf_defaults_cars
...
```
```{r}
rf_mod_cars <- rand_forest(trees=1000, mtry=14, min_n=60) %>% 
  set_engine("ranger") %>% 
  set_mode("regression")
```
```{r}
...
norm_recipe_cars <- 
  recipe(
    IR ~ FEE + MCAP + PE + TA + MNGR + Bull_Index + SECTOR_B + SECTOR_C + SECTOR_D + SECTOR_E + SECTOR_F + SECTOR_G + SECTOR_H + SECTOR_I, 
    data = cars
  ) %>%
  step_dummy(all_nominal()) %>%
  step_center(all_predictors()) %>%
...
lm_cars <- 
  ...
  set_engine("lm") %>%
    set_mode("regression") %>%
  fit(IR ~ FEE + MCAP + PE + TA + MNGR + Bull_Index + SECTOR_B + SECTOR_C + SECTOR_D + SECTOR_E + SECTOR_F + SECTOR_G + SECTOR_H + SECTOR_I, data = cars)
summary(lm_cars$fit)
...
test_results_cars <- 
  ...
  rename(`random forest` = .pred) %>%
  bind_cols(
    predict(lm_cars, new_data = test_normalized_cars) %>%
      rename(lm_cars = .pred)
  )
test_results_cars
```
...
```{r}
test_results_cars %>% 
  gather(model, prediction, -IR, -Bull_Index, -FEE, -MCAP, -MNGR, -PE, -SECTOR, -TA, -SECTOR_B, -SECTOR_C, -SECTOR_D, -SECTOR_E, -SECTOR_F, -SECTOR_G, -SECTOR_H, -SECTOR_I) %>% 
  ggplot(aes(x = prediction, y = IR)) + 
  geom_abline(col = "green", lty = 2) + 
  geom_point(alpha = .4) + 
  facet_wrap(~model) + theme_bw() + coord_fixed(xlim = c(-10, 40))
```
...
h + theme_classic() + scale_fill_brewer(palette="PRGn") + theme(legend.position="none") + geom_point(shape=21, size=3, fill="black") + scale_y_continuous(limits = c(15.8,21)) + coord_flip() 
```
...
r + coord_flip() + theme_classic() + scale_fill_brewer(palette="Blues") + theme(legend.position="none") + geom_point(shape=21, size=3, fill="black") + scale_y_continuous(limits = c(15.8,21))
```
